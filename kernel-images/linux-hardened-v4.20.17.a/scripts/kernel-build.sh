...
git clone https://github.com/anthraxx/linux-hardened.git linux-source-$KERNEL_SOURCE_VERSION

cp /root/kernel-config /root/linux-source-$KERNEL_SOURCE_VERSION/.config

cd /root/linux-source-$KERNEL_SOURCE_VERSION
git checkout $KERNEL_SOURCE_VERSION
yes '' | make oldconfig  && make -j $(nproc) vmlinux

echo "{\"machine\": \"$(uname -m)\", \"platform\": \"$(uname -i)\", \"version\": \"${KERNEL_SOURCE_VERSION}\", \"from\": \"github.com/anthraxx/linux-hardened.git\"}" > /output/_kernel_meta.json
